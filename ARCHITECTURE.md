# ARCHITECTURE.md — vLLM 部署与约束解码项目

## 一、项目定位

本项目通过 **6 个独立可运行的脚本**，把 vLLM 从"一个推理框架"拆解成开发者可以感性理解的具体能力：
...
场景选型：**金融问答 / 电商下单**——和相关项目中 `rag_annual_report`、`react_financial_agent` 等项目呼应，开发者能把 vLLM 的推理层和 RAG/Agent 的应用层串起来。
...
| vLLM OpenAI server | OpenAI 兼容、动态 batching、约束解码 | 只支持 Linux | **主线** |
...
- 0.5B 的"笨"恰好是优势——它会犯真实的错误（字段拼错、枚举值不匹配、JSON 断尾），让 guided_json 的价值被放大展示
...
| `--enforce-eager` | 加 | 关 CUDA graph 首次启动快 5~10 秒（不追求极致性能）|
...
Demo的"失败模式"精心设计（见 `demo_function_call.py`）：
...
- 分层指标：**JSON 合法 → 字段齐全 → 完整 schema 通过**，让开发者看清 3 种模式的差异到底在哪一层
